...
void rtpstream_play(rtpstream_callinfo_t* callinfo, rtpstream_actinfo_t* actioninfo)
{
    ...
    const int header_size = get_wav_header_size(taskinfo->new_file_bytes, taskinfo->new_file_size);
    if (header_size > 0 && taskinfo->new_file_size >= header_size) {
        taskinfo->new_file_bytes += header_size;
        taskinfo->new_file_size -= header_size;
    }
